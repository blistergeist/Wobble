...
def calculate_velocities(t, vel, acc):
    px = vel[0] + acc[0]*t
    py = vel[1] + acc[1]*t

    return np.array([px, py])

def dopri_calc_positions(bodies):
    t0 = 0
    dt = 86400*4  #1 day seconds
    t1 = dt*10
    vel = np.zeros(2)
    acc = np.zeros(2)
    posD = np.zeros(2)
    ...
    for body in bodies:
        dopriPos.append(ode(calculate_velocities).set_integrator('dopri5', nsteps=10000))
        dopriPos[i].set_f_params(acc)

        #set initial conditions
        vel[0] = body.vx
        vel[1] = body.vy
        dopriPos[i].set_initial_value(vel)
        i += 1
    ...
    while dopriPos.t < t1:
        for body in bodies:
            ...
            for other in bodies:
                ...
                total_fx += fx
                total_fy += fy
            posD = dopriPos[j].integrate(dopriPos[j].t+dt, dt)
            body.pxvector.append(posD[0])
            body.pyvector.append(posD[1])
